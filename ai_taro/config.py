...
# ============================================================
# Gemini API 設定（v4.30: 二段構え）
# ============================================================
# 相槌用（文脈レーン・ちょっかい・ボット通知など、回数が多い軽い仕事）
GEMINI_MODEL = "gemini-2.5-flash-lite"

# 会話用（呼びかけ・会話モード・視聴者コマンド・名指し反応・俳句/謎かけ）
# 2.5 Flash はLiteより賢く、無料枠も別勘定（2026年7月時点で約1500回/日）。
# 上位モデルに差し替え可能（将来の課金オプション）:
...
# --- v4.20: 手帳2.0 ---
# 手帳（learned_profile.json）を配信中に自動保存する間隔（秒）
# クラッシュや強制終了でその日の学習が消えるのを防ぐ
PROFILE_AUTOSAVE_SECONDS = 600

# --- v4.30: 会話継続モード ---
# 「太郎、〜」への応答後、この秒数以内の発言は会話の続きとして即応答する
# （キャッチボール成立のため。往復数の上限は CONVERSATION_MAX_TURNS と共用）
CONVERSATION_WINDOW_SECONDS = 30

# ============================================================
# 取材・記憶・検索設定（v4.50）
# ============================================================
# 呼び名の取材: 呼び名が未設定の視聴者に太郎が「なんてお呼びすれば？」と質問する
INTERVIEW_ENABLED = True
INTERVIEW_MAX_PER_STREAM = 3     # 1配信で質問する人数の上限
INTERVIEW_ANSWER_WINDOW = 120    # 回答を待つ秒数

# 「覚えて」コマンド: 呼びかけにこの語が含まれると直近の会話から手帳へ即時記録
REMEMBER_TRIGGERS = "覚えて,覚えた？,覚えた?,メモして,記録して"
# 記録の取り消し: 直前に覚えた内容を取り消す
FORGET_TRIGGERS = "登録しないで,覚えないで,取り消して"

# 検索機能（v4.50で復活）: 呼びかけにこの語が含まれるとGoogle検索して答える
# 例:「太郎、フォートナイトの新シーズンについて調べて」
# ※検索は特殊な課金（無料枠超過後は1回数円）のため、請求が気になったらFalseに
SEARCH_ENABLED = True
SEARCH_TRIGGERS = "調べて,検索して,ググって,について教えて"

# 名指し反応に会話用モデルを使うか（False=Liteで節約・掛け合いの質は少し落ちる）
MENTION_USE_SMART = True

# 俳句・謎かけイベント（v4.50: Lite・配信内容と関連付けない省エネ版に変更）
EVENTS_ENABLED = True
